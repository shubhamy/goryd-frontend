...
app.directive('genericCalendar' , function(){
  return {
    ...
    controller : function($scope){
      $scope.dayItems = [];
      $scope.dayPrice = [];
      for (var i=0;i<$scope.dateDisp.length;i++){
        $scope.dayItems[i] = new Date($scope.dateDisp[i].date);
        $scope.dayPrice[i] = $scope.dateDisp[i].price;
      }
      $scope.dateDisp = new Date("2016-05-15");
      ...
      $scope.showDay = function(input){
        ...
        if (datesMap.flags[input]=="Cur"){
          ...
          selectedDate = $scope.dates[input];
          $scope.gotoPrev();
